...
# Часть 1. Начало
{: .no_toc }  
Разработаем простую программу **mpg**, которая будет подключаться к базе данных PostgreSQL и выполнять простой запрос. Затем выполним рефакторинг программы. Рефакторинг у нас - это основная часть процесса разработки программного обеспечения.  
