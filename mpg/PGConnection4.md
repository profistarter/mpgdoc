...
```
Указываем пароль.  Для выхода набираем ```\q```. 
...
testdb=#
```
Если появилось приглашение ```testdb=#```, то все в порядке. Для выхода набираем ```\q```.    

Еще нам нужно установить бибилотеки C для работы с Postgre SQL. Набираем в консоли:  
```
...
```
Поясним аргументы компилятора ```g++```:
...
- далее идут папки "наших" заголовочных файлов;
