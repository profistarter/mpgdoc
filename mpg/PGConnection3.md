...
Мы поменяди метод ```exec()``` чтобы он тоже возвращал результат, чтобы тип возвращаемого результата был такой же как в методе ```receive()```. Метод ```exec()``` будет возвращать вектор с одним указателем на результат, так как результат всегда один, даже для для нескольких запросов объединенных в один.
```c++
