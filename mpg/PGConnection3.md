...
Примечательность этого способа в том, что для такой работы нужен всего лишь один поток, в отличие от многопоточной обработки. Остальные потоки освобождаются для другой полезной работы. Тем самым достигается еще большая производительность.  
...
Цикл выполняется пока не законцится очередь. Пока сделаем так, в будещем можем поменять на бесконечный цикл. Рассмотрим цикл пошагово:  
...
Шаги 4 и 5 работают независимо и мы можем смело поменять их местами.  
  
4. Для всех сокетов, готовых отдавать мы выполняем метод ```receive()``` класса ```PGConnection``` и идем дальше.  
...
#include <functional>
```
В класс ```PGConnection``` добавим два типа:
Первывй описывает результат выполнения запроса.
...
Свойство ```Results results``` необходимо хранить в соединении, потому-что результат нескольких запросов, объединенных в один возвращается по мере выполнения каждого запроса. Мы будем получать результат и записывать его в вектор ```results```.  
 ...
 И два метода очистки состояния отправки и выводи ошибки.  
...
```
Здесь мы инициализируем новые свойства, устанавливаем режим неблокирующего соединения и записываем номер сокета.
  
Мы поменяди метод ```exec()``` чтобы он тоже возвращал результат, чтобы тип возвращаемого результата был такой же как в методе ```receive()```. Метод ```exec()``` будеn возвращать вектор с одним указателем на результат, так как результат всегда один, даже для для нескольких запросов объединенных в один.
```c++
...
```c++
        ...
        res.reset(PQgetResult(connection.get()), &PQclear);
```
Используется часто для установки умного указателя, здесь создавая умный указатель мы передаем ему деструктор ```PQclear``` (см. [документацию к libpq](https://postgrespro.ru/docs/postgresql/9.6/libpq-exec#libpq-exec-main)), который вызовется, когда не останется ни одного умного указателя на результат.  
...
Если конец резултатам, то вызываем функцию обработки результата запроса и передаем ей вектор результатов.  
...
Метод ```loop()``` - главный метод этого класса. Работа этого метода подробно описана в предыдущем разделе.
...
Реадизация класса ```PGPool_Async``` следующая:
```c++
/* --------------------------------------------------- */
/*                   IMPLEMENTATION                    */
/* --------------------------------------------------- */
...
Метод ```for_each_conn()``` используется в методе  ```loop()``` дважды: перед вызовом функции ```select()``` для подготовки "наборов" файловых дескрипторов для передачи функции ```select()```; после вызова функции ```select()``` для обработки сокетов оставшихся в "наборах" (это сокеты готовые либо получать данные, либо отдавать данные).  
