...
Рабочий проект этого раздела можно найти [здесь](https://github.com/profistarter/mpg/tree/48a53edb5552f9fa38711b7471fa9e2751e6de60) (ссылка на коммит).
## Содержание
{: .no_toc }  
1. TOC
{:toc}
...
Создадим специальный класс ```PGPool_Threads```, который будет создавать потоки и передавать им соединения ```PGConnection```. Т.е. с каждым потоком будет связано одно соединение. Так как мы используем блокирующую функцию ```PGexec()``` из библиотеки ```libpq```, то поток будет приостанавливаться, пока сервер PostgreSQL не вернет ответ. Но пока один поток ждет другие потоки могут работать, т.е. отправлять запросы. Тем самым достигается параллельная работа нескольких соединений.   
Для передачи запросов соединениям все потоки используют одну очередь заданий.  
...
| параллельно 3 потоков          | 0.010957      | 0.13297       | 0.269058      |
...
Какой результат мы получим, когда добавим асинхронную обработку заданий в очереди?
