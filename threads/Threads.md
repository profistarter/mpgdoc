...
# Часть 1. Пул потоков
{: .no_toc }  
Необходимость создания пула потоков возникла в связи с ускорением запросов к PostgreSQL. В процессе работы над разделом [Используем многопоточность](../mpg/PGConnection2.md) мы разделили класс ```PGPool_Threads``` (Пул соединений) на два, выделив базовый класс ```Treads``` (Потоки). Следуем основному принципу: Повторное использование кода.  
## Содержание
{: .no_toc }  
1. TOC
{:toc}
# Класс Threads - пул потоков
Создаем папку ```treads```. Папки с подсистемами располагаем на одном уровне: те имеем главную папку ``Мои Проекты```. В ней  находятся папки ```threads```, ```mpg```, ```config```, ```_include``` и другие.  
