...
protected:
    ...
    void loop(Loop_Fn _fn, Args... args);
```
- Метод ```loop()``` - это цикл, который выполняется в потоку. Он получает задание, вызывая метод ```get_task()```, и передает это задание функции ```_fn```, вместе с аргументами ```args```.
