...
Создаем папку ```treads```. Папки с подсистемами располагаем на одном уровне: те имеем главную папку ```Мои Проекты```. В ней  находятся папки ```threads```, ```mpg```, ```_config```, ```_include``` и другие.  
...
Для тестирования используем библиотеку doctest. Как ее подключить смотрите [здесь](../life_hack/add_doctest.md). Еще, в тестах будем использовать ```std::chrono_literals```, поэтому подключаем библиотеку ```chrono```:  
```c++
...
В секцию TESTS добавим проверку. Перепишем ```TEST_CASE("Количество вызовов задач")```:  
```c++
    ...
