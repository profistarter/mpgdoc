...
```
Если в проекте уже есть task.json, то добавляем еще одну задачу, можно просто скопировать и меняем атрибут ```"label"``` - это название задачи. Назовем ее ```"tests"```. Также меняем атрибут ```"args"``` как описано выше.
```json
{
    "version": "2.0.0",
    "tasks": [
...
```
Если в проекте уже есть launch.json, то добавляем еще одну конфигурацию, можно просто скопировать и меняем атрибут ```"name"```, ```"program"``` и ```"preLaunchTask"```. Это нужно чтобы отлаживать тесты.   
