...
Модуль ```utils``` будет использоваться в составе разрабатываемых приложений. Как уже упоминалось папка модуля ```utils``` расположена на одном уровне с папками приложений и других модулей. Необходимо указать компилятору приложения, например ```mpg``` откуда брать модуль ```utils```. Для этого в папке модуля ```mpg/.vscode``` откройте файл ```task.json``` и добавьте в него аргументы компилятора как показано на рисунке:  
[include_utils](../img/include_utils.png)
На этом описание преобразования кодовой страницы завершено. Далее идет второй способ, который используется если программа будет работать только в windows.
